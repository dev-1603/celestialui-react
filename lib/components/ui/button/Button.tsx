  ...
  (
    ...
    const content = (
      <>
        ...
        {leftIcon && !loading && (
          <Icon name={leftIcon} className="cui-button__icon cui-button__icon--left" />
        )}

        {!iconOnly && <span className="cui-button__content">{children}</span>}

        {rightIcon && !loading && (
          <Icon name={rightIcon} className="cui-button__icon cui-button__icon--right" />
        )}
      </>
    )
    ...
    if (href) {
      return (
        <Component {...commonProps} href={href} target={target} rel={rel}>
          {content}
        </Component>
      )
    }
    ...
    return (
      <Component {...commonProps} ref={ref} type={type} disabled={disabled || loading}>
        {content}
      </Component>
    )
  },
